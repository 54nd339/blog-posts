...
The loader (`main-site/lib/content/blog.ts`) walks every category directory
**recursively**, so posts may sit in subcategory folders
(`physics/optics/wave-optics.md`). It reads `title`, `date`, `tags`,
`description`, `draft`, and `slug`. **The route is derived from `slug`**, not the
file path: `slug: /physics/vectors` → `/citadel/physics/vectors`. So `slug` is
load-bearing — never change it — and **the slug tail is the filename stem**
(`.../vectors.md` ↔ `slug: /physics/vectors`), independent of which subfolder the
file lives in.
...
**Load-bearing.** Format: `slug: /<category>/<name>`, where `<category>` is the
top-level directory and `<name>` is the filename stem — **not** the full path, so
a post in a subcategory folder keeps a flat two-segment slug
(`physics/optics/wave-optics.md` → `slug: /physics/wave-optics`). Served by the
`/citadel/[...slug]` catch-all — e.g. `slug: /physics/rotations` lives at
`/citadel/physics/rotations`. Never change an existing post's `slug`; it is what
inbound links point at.

- Slug tail **is** the filename stem: `.../rotations.md` → `/physics/rotations`.
...
- **Routing is by `slug`.** URL = `slug` under `/citadel`, served by `app/citadel/[...slug]/` (a catch-all). `slug: /physics/rotations` → `/citadel/physics/rotations`. Changing a `slug` changes the URL; that's why slugs are frozen. The loader walks each category **recursively**, so on-disk subcategory folders (`physics/optics/…`) do not affect the URL.
- **Slugs must be unique repo-wide** (the catch-all resolves the first post whose slug matches). Format is always `/<category>/<filename-stem>` — the top-level category plus the file's own stem, never the intervening subfolder — so uniqueness follows the filename, not the full path.
